...
class Bot():
    ...
    def core(self): 
        ...
        data: list = self.update_json() # TODO maybe this function is called twice because of non_business_day()

        venta_blue: float = float(data["venta"].replace(",","."))
        now_change: str = datetime.now().strftime("%d/%m/%y - %H:%M:%S")

        if intervalo_blue > venta_blue:
            self.telegram_client.send_message(chat_id=self.channel_id, text=f"El Dólar Blue bajó ${int(intervalo_blue - venta_blue)} a ${int(venta_blue)}.")
            self.twitter_client.create_tweet(text=f"El Dólar Blue bajó ${int(intervalo_blue - venta_blue)} a ${int(venta_blue)}.") 
            intervalo_blue = venta_blue
            print(f"{now_change} - Went down to ${int(intervalo_blue)} ARS/USD")
            
        elif intervalo_blue < venta_blue:
            self.telegram_client.send_message(chat_id=self.channel_id, text=f"El Dólar Blue subió ${int(venta_blue - intervalo_blue)} a ${int(venta_blue)}.")
            self.twitter_client.create_tweet(text=f"El Dólar Blue subió ${int(venta_blue - intervalo_blue)} a ${int(venta_blue)}.")
            intervalo_blue = venta_blue
    ...
    def closing(self): # 16.30hs
        ...
        if self.non_business_day():
            now_open: str = datetime.now().strftime("%d/%m/%y - %H:%M:%S")
            print(f"{now_open} - Non-business day, so no activity")
            return
        ...
        if cierre_blue > apertura_blue:
            self.telegram_client.send_message(chat_id=self.channel_id, text=f"El Dólar Blue cierra la jornada a ${int(cierre_blue)}. En el día subió ${int(cierre_blue - apertura_blue)}, lo que significa una variación de {variation}.")
            self.twitter_client.create_tweet(text=f"El Dólar Blue cierra la jornada a ${int(cierre_blue)}. En el día subió ${int(cierre_blue - apertura_blue)}, lo que significa una variación de {variation}.")
            print(f"{now_open} - Closed at {cierre_blue} ARS/USD")

        elif cierre_blue < apertura_blue:
            self.telegram_client.send_message(chat_id=self.channel_id, text=f"El Dólar Blue cierra la jornada a ${int(cierre_blue)}. En el día bajó ${int(apertura_blue - cierre_blue)}, lo que significa una variación de {variation}.")
            self.twitter_client.create_tweet(text=f"El Dólar Blue cierra la jornada a ${int(cierre_blue)}. En el día bajó ${int(apertura_blue - cierre_blue)}, lo que significa una variación de {variation}.")
            
            print(f"{now_open} - Closed at {cierre_blue} ARS/USD")

        else:
            self.telegram_client.send_message(chat_id=self.channel_id, text=f"El Dólar Blue cierra la jornada sin cambios a ${int(cierre_blue)}.")
            self.twitter_client.create_tweet(text=f"El Dólar Blue cierra la jornada sin cambios a ${int(cierre_blue)}.")
